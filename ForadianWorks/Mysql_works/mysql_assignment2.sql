...
a)	select students.first_name, students.last_name, dayname(s1.month_date) as 'Day of Absent' ,e1.first_name as 'Teacher'
	...
	where students.school_id = 32
	group by s1.subject_id;
...
a)	select first_name,last_name, f2.name as 'Fee Category', f1.balance as 'amount', f4.name as 'Fine Category', f3.fine_amount as 'Fine Amount', f3.fine_days as 'Fine Days'
	from students
	...
	inner join fine_rules as f3 on f2.fine_id = f3.id
	inner join fines as f4 on f3.fine_id = f4.id
	where students.school_id = 83 and f1.is_paid = 0 and (first_name like '%aman%' or middle_name like '%aman%' or last_name like '%aman%')
	group by f2.id;
...
a)	select first_name, last_name, b1.user_id, f1.title, b1.book_id
	...
	inner join finance_transactions as f1 on b1.book_id = f1.id
	where students.school_id = 83 and b1.status != 'Returned'
	group by b1.user_id;
...
a)	select first_name,last_name, f2.name as 'Fee Category', f1.balance as 'Amount to pay', f4.name as 'Fine Category', f3.fine_amount as 'Fine Amount', f3.fine_days as 'Fine Days'
	from students
	inner join finance_fees as f1 on f1.student_id = students.id
	inner join finance_fee_collections as f2 on f2.id = f1.fee_collection_id
	inner join fine_rules as f3 on f2.fine_id = f3.id
...
a)	select f2.name as 'Fee Category', finance_fees.balance as 'Amount', f4.name as 'Fee particular', f4.amount as 'Amount', f3.name as 'Discount Category', f3.discount as 'Discount Amount', finance_fees.student_id, f5.name, f6.fine_amount, f6.fine_days
	from finance_fees
