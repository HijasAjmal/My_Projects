...
time[] - Refernce time of a page loaded in a particular frame. 
The page referenced long ago will have a low time value.
The page with smallest time value is the FIFO page
...
int main()
{
	...
	printf ("\n Reference to Page %d has come ",pages[i]); 
		...
		//printf("\nFrame j- %d =%d and page i- %d=%d",j,frames[j],i,pages[i]);
            ...
            if(frames[j] == pages[i]){		//Page is already loaded in frame-j		
                timer++;
		...
		printf("\nTry to find free frame and load page");	
            for(j = 0; j < no_of_frames; ++j){
                if(frames[j] == -1){		//check for free frame
                    ...
                    frames[j] = pages[i];
                    time[j] = timer;
			printf("\n Page loaded to free Frame-%d at time %d",j,timer);
                    flag2 = 1;
                    break;
                }
            }    
        ...
        if(flag2 == 0){				//find lru page and replace it
            ...
            pos = findFIFO(time, no_of_frames);
            timer++;
            faults++;
            frames[pos] = pages[i];
            time[pos] = timer;
		printf("\n Replace the page loaded into frame-%d . time =%d",pos,timer);
        }
